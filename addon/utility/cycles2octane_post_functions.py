...
def OctaneUniversalMaterial(new_node, old_node):
    ...
    if old_node.inputs['Emission Color'].is_linked:
        # Get the connected texture node
        emission_texture = old_node.inputs['Emission Color'].links[0].from_node
        
        # Create OctaneBlackBodyEmission node
        ...
        # Get existing texture node (should be OctaneRGBImage after conversion)
        for node in new_node.id_data.nodes:
            if hasattr(node, 'image') and node.image == emission_texture.image:
        ...
        # Connect emission node to universal material
        create_node_link(
            new_node.id_data,
