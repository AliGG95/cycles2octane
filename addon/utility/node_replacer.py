    ...
    def _replace_node(self, node: Node, null_node: NullNode, custom_node_group: CustomNodeGroup, replace_node_data: ReplaceNodeData) -> Node:
        ...
        if null_node:
            ...
            if replace_node_data.convert_to_node:
                ...
                # Specjalna obsługa node'ów tekstury
                if node.bl_idname == 'ShaderNodeTexImage' and replace_node_data.convert_to_node == 'OctaneRGBImage':
                    if hasattr(node, 'image') and node.image:
                        # Bezpośrednie przeniesienie obrazu
                        ...
                        try:
                            if node.image.colorspace_settings.name == 'Non-Color':
                                ...
                                new_node.inputs['Legacy gamma'].default_value = 2.2
                        except:
                            pass
                            
                        # Dodanie innych specyficznych ustawień dla tekstury
                        new_node.inputs['Power'].default_value = 1.0
                        new_node.inputs['Linear sRGB invert'].default_value = True
                        new_node.inputs['Invert'].default_value = False
                        
                        # Ustaw tryby powtarzania tekstury
                        if hasattr(node, 'extension'):
                            if node.extension == 'REPEAT':
                                new_node.inputs['Border mode (U)'].default_value = "Wrap around"
                                new_node.inputs['Border mode (V)'].default_value = "Wrap around"
                            elif node.extension == 'EXTEND':
                                new_node.inputs['Border mode (U)'].default_value = "Clamp value"
                                new_node.inputs['Border mode (V)'].default_value = "Clamp value"
            else:
                return

        new_node.location = node.location
        new_node.name = 'OCT_' + node.name
        new_node.label = node.label if node.label else node.name
